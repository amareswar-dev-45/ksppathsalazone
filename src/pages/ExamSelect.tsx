...
const ExamSelect = () => {
  ...
  const { data: exams = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data: examData, error } = await supabase.from("exams").select("*").order("mock_number");
      if (error) throw error;
      // Get question data per exam
      const { data: questions } = await supabase.from("questions").select("exam_id, marks, negative_marks");
  ...
  return (
    <div className="min-h-screen bg-background p-4">
      <div className="max-w-lg mx-auto animate-fade-in">
        <button onClick={() => navigate("/student")} className="flex items-center gap-2 text-muted-foreground hover:text-foreground mb-6 transition-colors">
        ...
        {isLoading ? (
          ...
          <p className="text-muted-foreground text-center py-8">No exams available yet.</p>
        ) : (
